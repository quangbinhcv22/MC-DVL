    ...
    public class EnhancedScroller : MonoBehaviour
    {
        ...
        )
        {
            ...
            if (loop)
            {
                ...
                var set2Position =
                    GetScrollPositionForCellViewIndex(dataIndex + NumberOfCells, CellViewPositionEnum.Before) + offset;
                var set3Position =
                    GetScrollPositionForCellViewIndex(dataIndex + (NumberOfCells * 2), CellViewPositionEnum.Before) +
                    offset;
            ...
            // start tweening
            StartCoroutine(TweenPosition(tweenType, tweenTime, ScrollPosition,
                newScrollPosition, jumpComplete));
        }
        ...
        public EnhancedScrollerCellView GetCellViewAtDatIndex(int dataIndex)
        {
            for (int i = 0; i < _activeCellViews.Count; i++)
            {
                var cellView = _activeCellViews[i];
                if (cellView.dataIndex == dataIndex) return cellView;
            }
        ...
        private void _Resize(bool keepPosition)
        {
            ...
            if (scrollDirection == ScrollDirectionEnum.Vertical)
                _container.sizeDelta = new Vector2(_container.sizeDelta.x,
                    _cellViewOffsetArray.Last() + padding.top + padding.bottom);
            else
                _container.sizeDelta = new Vector2(_cellViewOffsetArray.Last() + padding.left + padding.right,
        ...
        private int _GetCellIndexAtPosition(float position, int startIndex, int endIndex)
        {
            ...
            if ((_cellViewOffsetArray[middleIndex] +
                 (scrollDirection == ScrollDirectionEnum.Vertical ? padding.top : padding.left)) >= position)
        ...
        [Header("Layout Group Align")] public TextAnchor typeAnchor = TextAnchor.UpperCenter;
        [Header("Container Position")] public TextAnchor containerPos;

        /// <summary>
        /// Caches and initializes the scroller
        /// </summary>
        public GameObject _containerCache;
        ...
        void Awake()
        {
            ...
            if (containerPos == TextAnchor.MiddleCenter)
            {
            ...
            _layoutGroup.childAlignment = typeAnchor;
        ...
        private RectTransform rectTransformScroller;

        /// <summary>
        ///  chỉ dùng trong tình huống vertical scroller list column
        /// </summary>
        public void JumpToItem(int index, int itemPerRow, float jumpTime)
        {
            if (rectTransformScroller == null)
                rectTransformScroller = _scrollRect.content.gameObject.transform.GetComponent<RectTransform>();
            int amount = itemPerRow * NumberOfCells;
            int itemIndexWithRow = amount;
            while (itemIndexWithRow > index)
            {
                itemIndexWithRow -= itemPerRow;
            }

            int rowIndex;
            if (itemIndexWithRow < itemPerRow) itemIndexWithRow = itemPerRow;

            if (itemIndexWithRow % itemPerRow != 0) rowIndex = Mathf.CeilToInt(itemIndexWithRow / itemPerRow);
            else rowIndex = itemIndexWithRow <= itemPerRow ? 0 : itemIndexWithRow / itemPerRow;

            float yPos = (ScrollRectSize / NumberOfCells) * rowIndex / NumberOfCells;

           // rectTransformScroller.DOAnchorPos(new Vector2(rectTransformScroller.anchoredPosition.x, yPos), jumpTime);
        }


        public void ShakeScroller()
        {
            if (rectTransformScroller == null)
                rectTransformScroller = _scrollRect.content.gameObject.transform.GetComponent<RectTransform>();
            if (rectTransformScroller == null) return;
            rectTransformScroller.anchoredPosition =
                new Vector2(rectTransformScroller.anchoredPosition.x, ScrollRectSize);
            //rectTransformScroller.DOAnchorPos(new Vector2(rectTransformScroller.anchoredPosition.x, 0), 0.8f);
        }
